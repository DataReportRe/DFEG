...
if __name__ == "__main__":
  ...
  parser.add_argument('--ablation', default='dfeg', help='ablation analysis: WOFP , WORAND, WOMCMC, WBGRT, WBO')
  ...
  for i in functions_to_test:
    ...
    if args.dfeg_sampling:
      ...
      if num_inputs==1:
        signal.alarm(2*num_inputs)
      elif num_inputs<4:
        signal.alarm(4*num_inputs)
      elif num_inputs>=5:
      ...
      try:
          if ablation == "dfeg":
            ...
            analysis.optimize_dfegly_WOFP(shared_lib, inputs, max_iters, unbounded,rd,rid)
          elif ablation == "WORAND":
            analysis.optimize_dfegly_WORAND(shared_lib, inputs, max_iters, unbounded,rd,rid)
          elif ablation == "WOMCMC":
            analysis.optimize_dfegly_WOMCMC(shared_lib, inputs, max_iters, unbounded,rd,rid)
          elif ablation == "WBGRT":
            analysis.optimize_dfegly_WBGRT(shared_lib, inputs, max_iters, unbounded,rd,rid)
          elif ablation == "WBO":
            analysis.optimize_dfegly_WBO(shared_lib, inputs, max_iters, unbounded,rd,rid)
      finally:
          signal.alarm(0)
      signal.alarm(0)
  ...
  if rid == 101:
    save_line_list("results/dfeg_res_ablation/dfeg_res_"+ablation+".pkl",results_save)
  else:
    save_line_list("results/dfeg_res/dfeg_res_"+ablation+str(rid)+".pkl",results_save)
