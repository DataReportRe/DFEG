...
if __name__ == "__main__":
  ...
  parser.add_argument('--defped_sampling', action='store_true', help='Use defped sampling')
  ...
  for i in functions_to_test:
    ...
    # DEFPED Sampling
    if args.defped_sampling:
      #print('******* DEFPED SAMPLING on:', shared_lib)
      inputs = num_inputs
      max_iters = 30 * int(math.pow(18, inputs))
      unbounded = True
      start_time = time.time()
      bo_analysis.optimize_Defpedly(shared_lib, inputs, max_iters, unbounded,rd,rid)
      bo_analysis.print_results_defped(shared_lib)
      inputs_res = list(bo_analysis.defped_inp_results.items())
      inputs_res_mc = list(bo_analysis.defped_inp_results_mc.items())
      bo_analysis.defped_inp_results = {"inf+":[],"inf-":[],"sub+":[],"sub-":[],"nan":[]}
      bo_analysis.defped_inp_results_mc = {"inf+":[],"inf-":[],"sub+":[],"sub-":[],"nan":[]}
      end_time = time.time()
      exec_time = end_time-start_time
      print("time is "+str(end_time-start_time))
      if type(i) is FunctionSignature:
          results_save.append([list(bo_analysis.defped_results.items()),exec_time,i.fun_name,inputs_res,list(bo_analysis.defped_results_mc.items()),inputs_res_mc,bo_analysis.fine_search_time])
      elif type(i) is SharedLib:
          results_save.append([list(bo_analysis.defped_results.items()),exec_time,i.path,inputs_res,list(bo_analysis.defped_results_mc.items()),inputs_res_mc,bo_analysis.fine_search_time])
      bo_analysis.defped_results = {}
      bo_analysis.defped_results_mc = {}
  save_line_list("results/dfeg_bo_res/dfeg_bo_res.pkl",results_save)
  print(inps_num)
  #print(results_save)
