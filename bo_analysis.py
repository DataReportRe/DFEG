...
random_results = {}
defped_results = {}
defped_inp_results = {"inf+":[],"inf-":[],"sub+":[],"sub-":[],"nan":[]}
defped_results_mc = {}
defped_inp_results_mc = {"inf+":[],"inf-":[],"sub+":[],"sub-":[],"nan":[]}
defped_results_de = {}
defped_inp_results_de = {"inf+":[],"inf-":[],"sub+":[],"sub-":[],"nan":[]}
...
def run_optimizer_dfeg(bounds, func, exp_name,unbounded,ori_func):
  ...
  try:
    ...
    for _ in range(bo_iterations):
      ...
      if target == 0.0:
        val = ori_func(*inps)
        save_results_defped(val, exp_name, unbounded,inps)
        save_results_defped_mcmc(val, exp_name, unbounded,inps)
...
def run_optimizer(bounds, func, exp_name):
  ...
  save_results_defped(r, exp_name, unbounded,inp)
  save_results_defped_mcmc(r, exp_name, unbounded,inp)
...
def print_results(shared_lib: str, number_sampling, range_splitting):
  ...
  print('')
# --------------- Defped Optimizer -------------
def save_results_defped(val: float, exp_name: str, unbounded: bool, inp):
  found = False
  # Infinity
  if math.isinf(val):
    if exp_name not in defped_results.keys():
      if val > 0.0:
        defped_results[exp_name] = [1, 0, 0, 0, 0]
        defped_inp_results["inf+"].append(inp)
        found = True
      else:
        defped_results[exp_name] = [0, 1, 0, 0, 0]
        defped_inp_results["inf-"].append(inp)
        found = True
    else:
      if val > 0.0:
        defped_results[exp_name][0] += 1
        defped_inp_results["inf+"].append(inp)
        found = True
      else:
        defped_results[exp_name][1] += 1
        defped_inp_results["inf-"].append(inp)
        found = True
  ...
  if numpy.isfinite(val):
    if val > -f64min and val < f64min:
      if val != 0.0 and val !=-0.0:
        if exp_name not in defped_results.keys():
          if val > 0.0:
            defped_results[exp_name] = [0, 0, 1, 0, 0]
            defped_inp_results["sub+"].append(inp)
            found = True
          else:
            defped_results[exp_name] = [0, 0, 0, 1, 0]
            defped_inp_results["sub-"].append(inp)
            found = True
        else:
          if val > 0.0:
            defped_results[exp_name][2] += 1
            defped_inp_results["sub+"].append(inp)
            found = True
          else:
            defped_results[exp_name][3] += 1
            defped_inp_results["sub-"].append(inp)
            found = True

  if math.isnan(val):
    if exp_name not in defped_results.keys():
      defped_results[exp_name] = [0, 0, 0, 0, 1]
      defped_inp_results["nan"].append(inp)
      found = True
    else:
      defped_results[exp_name][4] += 1
      defped_inp_results["nan"].append(inp)
      found = True

  if exp_name not in defped_results.keys():
    defped_results[exp_name] = [0,0,0,0,0]
  ...
  if unbounded:
    ...
    return found
  #return False
def save_results_defped_de(val: float, exp_name: str, unbounded: bool, inp):
  found = False
  # Infinity
  if math.isinf(val):
    if exp_name not in defped_results_de.keys():
      if val > 0.0:
        defped_results_de[exp_name] = [1, 0, 0, 0, 0]
        defped_inp_results_de["inf+"].append(inp)
        found = True
      else:
        defped_results_de[exp_name] = [0, 1, 0, 0, 0]
        defped_inp_results_de["inf-"].append(inp)
        found = True
    else:
      if val > 0.0:
        defped_results_de[exp_name][0] += 1
        defped_inp_results_de["inf+"].append(inp)
        found = True
      else:
        defped_results_de[exp_name][1] += 1
        defped_inp_results_de["inf-"].append(inp)
        found = True
  ...
  if numpy.isfinite(val):
    if val > -f64min and val < f64min:
      if val != 0.0 and val !=-0.0:
        if exp_name not in defped_results_de.keys():
          if val > 0.0:
            defped_results_de[exp_name] = [0, 0, 1, 0, 0]
            defped_inp_results_de["sub+"].append(inp)
            found = True
          else:
            defped_results_de[exp_name] = [0, 0, 0, 1, 0]
            defped_inp_results_de["sub-"].append(inp)
            found = True
        else:
          if val > 0.0:
            defped_results_de[exp_name][2] += 1
            defped_inp_results_de["sub+"].append(inp)
            found = True
          else:
            defped_results_de[exp_name][3] += 1
            defped_inp_results_de["sub-"].append(inp)
            found = True

  if math.isnan(val):
    if exp_name not in defped_results_de.keys():
      defped_results_de[exp_name] = [0, 0, 0, 0, 1]
      defped_inp_results_de["nan"].append(inp)
      found = True
    else:
      defped_results_de[exp_name][4] += 1
      defped_inp_results_de["nan"].append(inp)
      found = True

  if exp_name not in defped_results_de.keys():
    defped_results_de[exp_name] = [0,0,0,0,0]
  ...
  if unbounded:
    ...
    return found
  #return False
def save_results_defped_mcmc(val: float, exp_name: str, unbounded: bool, inp):
  found = False
  # Infinity
  if math.isinf(val):
    if exp_name not in defped_results_mc.keys():
      if val > 0.0:
        defped_results_mc[exp_name] = [1, 0, 0, 0, 0]
        defped_inp_results_mc["inf+"].append(inp)
        found = True
      else:
        defped_results_mc[exp_name] = [0, 1, 0, 0, 0]
        defped_inp_results_mc["inf-"].append(inp)
        found = True
    else:
      if val > 0.0:
        defped_results_mc[exp_name][0] += 1
        defped_inp_results_mc["inf+"].append(inp)
        found = True
      else:
        defped_results_mc[exp_name][1] += 1
        defped_inp_results_mc["inf-"].append(inp)
        found = True
  ...
  if numpy.isfinite(val):
    if val > -f64min and val < f64min:
      if val != 0.0 and val !=-0.0:
        if exp_name not in defped_results_mc.keys():
          if val > 0.0:
            defped_results_mc[exp_name] = [0, 0, 1, 0, 0]
            defped_inp_results_mc["sub+"].append(inp)
            found = True
          else:
            defped_results_mc[exp_name] = [0, 0, 0, 1, 0]
            defped_inp_results_mc["sub-"].append(inp)
            found = True
        else:
          if val > 0.0:
            defped_results_mc[exp_name][2] += 1
            defped_inp_results_mc["sub+"].append(inp)
            found = True
          else:
            defped_results_mc[exp_name][3] += 1
            defped_inp_results_mc["sub-"].append(inp)
            found = True

  if math.isnan(val):
    if exp_name not in defped_results_mc.keys():
      defped_results_mc[exp_name] = [0, 0, 0, 0, 1]
      defped_inp_results_mc["nan"].append(inp)
      found = True
    else:
      defped_results_mc[exp_name][4] += 1
      defped_inp_results_mc["nan"].append(inp)
      found = True

  if exp_name not in defped_results_mc.keys():
    defped_results_mc[exp_name] = [0,0,0,0,0]
...
def print_results_defped(shared_lib):
  key = shared_lib+'|DEFPED'
  fun_name = os.path.basename(shared_lib)
  print('-------------- Results --------------')
  print(fun_name)
  if key in defped_results.keys():
    print('\tINF+:', defped_results[key][0])
    print('\tINF-:', defped_results[key][1])
    print('\tSUB+:', defped_results[key][2])
    print('\tSUB-:', defped_results[key][3])
    print('\tNaN :', defped_results[key][4])
  else:
    print('\tINF+:', 0)
    print('\tINF-:', 0)
    print('\tSUB+:', 0)
  ...
  print('')
  print('--------------MCMC Results --------------')
  if key in defped_results_mc.keys():
    print('\tINF+:', defped_results_mc[key][0])
    print('\tINF-:', defped_results_mc[key][1])
    print('\tSUB+:', defped_results_mc[key][2])
    print('\tSUB-:', defped_results_mc[key][3])
    print('\tNaN :', defped_results_mc[key][4])
  else:
    print('\tINF+:', 0)
    print('\tINF-:', 0)
    print('\tSUB+:', 0)
  ...
  print('')
  #print('--------------DE Results --------------')
  #if key in defped_results_de.keys():
  #  print('\tINF+:', defped_results_de[key][0])
  #  print('\tINF-:', defped_results_de[key][1])
  #  print('\tSUB+:', defped_results_de[key][2])
  #  print('\tSUB-:', defped_results_de[key][3])
  #  print('\tNaN :', defped_results_de[key][4])
  #else:
  #  print('\tINF+:', 0)
  #  print('\tINF-:', 0)
  #  print('\tSUB+:', 0)
...
def optimize_Defpedly_xs(shared_lib: str, num_inputs: int, max_iters: int, unbounded: bool, rand_seed: int,res_cnt: int):
    global CUDA_LIB
    CUDA_LIB = shared_lib
    exp_name = shared_lib+'|'+'DEFPED'
    if num_inputs == 1:
...
def optimize_Defpedly(shared_lib: str, num_inputs: int, max_iters: int, unbounded: bool, rand_seed: int,res_cnt: int):
    global CUDA_LIB
    CUDA_LIB = shared_lib
    exp_name = shared_lib+'|'+'DEFPED'
    if num_inputs == 1:
    ...
    for i in bounds:
        ...
        #inps = produce_n_input(i,8/num_inputs)
    ...
    for inps in all_inps:
      ...
      for inp in inps[0]:
          ...
          if res == 0.0: 
            found = save_results_defped(val, exp_name, unbounded,inp)
            trig_flag = 1
          else:
            temp_sum.append(res) 
      if trig_flag == 0:
